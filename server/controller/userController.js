...
const forgetPassword = async (req, res) => {
    try {
      ...
      <a href="http://localhost:3500/reset/${token}">http://localhost:3500/reset/${token}</a>
...
const resetPassword = async (req, res) => {
    try {
        ...
        if (req.body.newPassword.length < 8) return res.status(400).json({ 'message': 'password must have minimun length of 8 characters' });
...
module.exports = { handleLogin, handleSignup, handleLogout, forgetPassword, resetPassword };